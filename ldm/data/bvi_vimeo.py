...
from FlowFormerPlusPlus.visualize_flow import compute_flow, build_model
import torch
...
class Vimeo90k_triplet(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # frame3 and frame5 run flowformer
        flow = compute_flow(model, frame3, frame5)

        return {'image': frame4, 'prev_frame': frame3, 'next_frame': frame5, "hint": flow}
...
class BVIDVC_triplet(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # flowformer
        image1 = np.array(frame3).astype(np.uint8)[..., :3]
        image2 = np.array(frame5).astype(np.uint8)[..., :3]
        # if not keep_size:
        ...
        flow = compute_flow(self.model, image1, image2)
